...
binmode STDOUT, ':utf8';
use vars
	qw($opt_N $opt_a $opt_b $opt_c $opt_d $opt_e $opt_f $opt_h $opt_H $opt_m $opt_n $opt_p $opt_s $opt_t $opt_l $opt_v $opt_w $opt_2 $opt_u $opt_x $opt_U $opt_3 $opt_M $opt_z);
...
my $usage = "$0\n         [-a (evaluate all systems)] 
         ...
         <ROUGE-eval-config-file> [<systemID>]
";
getopts('N:ahHb:c:de:f:l:mMn:p:st:r:2:3:w:uUvxz:');
my $systemID;
...
# load stopwords
%stopwords = ();
open(STOP, '<:utf8', $stopwords) || die "Cannot open $stopwords\n";
...
if (-e "$wordnetDB")
{
	tie %exceptiondb, 'DB_File', "$wordnetDB", O_RDONLY, 0440, $DB_HASH
		or die "Cannot open exception db file for reading: $wordnetDB\n";
}
else
{
...
{
	open($doc, '<:utf8', $ARGV[0]) || die "Cannot open $ARGV[0]\n";
}
%ROUGEEvals       = ();
@ROUGEEvalIDs     = ();
%ROUGEPeerIDTable = ();
...
sub computeAverages
{
	...
	if (!defined($opt_t) || $opt_t == 1)
	{
		...
		$ROUGEAverages->{"CIAvgL_R"} =
			sprintf("%7.5f", $ResamplingArray[0][$ciLa] + ($ResamplingArray[0][$ciLb] - $ResamplingArray[0][$ciLa]) * $ciR);
		$ROUGEAverages->{"CIAvgU_R"} =
			sprintf("%7.5f", $ResamplingArray[0][$ciUa] + ($ResamplingArray[0][$ciUb] - $ResamplingArray[0][$ciUa]) * $ciR);
		...
		# find condifence intervals; take maximum distance from the mean
		$ROUGEAverages->{"CIAvgL_P"} =
			sprintf("%7.5f", $ResamplingArray[1][$ciLa] + ($ResamplingArray[1][$ciLb] - $ResamplingArray[1][$ciLa]) * $ciR);
		$ROUGEAverages->{"CIAvgU_P"} =
			sprintf("%7.5f", $ResamplingArray[1][$ciUa] + ($ResamplingArray[1][$ciUb] - $ResamplingArray[1][$ciUa]) * $ciR);
		...
		# find condifence intervals; take maximum distance from the mean
		$ROUGEAverages->{"CIAvgL_F"} =
			sprintf("%7.5f", $ResamplingArray[2][$ciLa] + ($ResamplingArray[2][$ciLb] - $ResamplingArray[2][$ciLa]) * $ciR);
		$ROUGEAverages->{"CIAvgU_F"} =
			sprintf("%7.5f", $ResamplingArray[2][$ciUa] + ($ResamplingArray[2][$ciUb] - $ResamplingArray[2][$ciUa]) * $ciR);
...
sub computeROUGEX
{
	...
	# evaluate peer
	{
		...
		{
			&computeSkipBigramScore(\@modelPaths, $peerPath, \@results, $skipDistance, $lengthLimit, $byteLimit, $inputFormat, $scoreMode,
															$alpha);
		}
		elsif ($metric eq "BE")
		{
...
sub readText
{
	...
	$done           = 0;
	@sntList        = ();
	open(TEXT, '<:utf8', $inPath) || die "Cannot open $inPath\n";
	if ($type =~ /^SEE$/oi)
	{
	...
	{      # ISI standard sentence by sentence format
		while (defined($line = <TEXT>))
		{
	...
	{    # SPL one Sentence Per Line format
		while (defined($line = <TEXT>))
		{
			chomp($line);
	...
	elsif ($lengthLimit != 0)
	{
		...
		foreach $s (@sntList)
		{
			...
			#@tokens = split(/\s+/, $s);
			@tokens = getListOfTokensInText($s);
			$sLen   = scalar @tokens;
	...
	if (defined($$tokenizedText))
	{
		...
		#$$tokenizedText =~ s/\s+$//;
		#$$tokenizedText =~ s/\s+/ /g;
		$$tokenizedText = join(' ', getListOfTokensInText($$tokenizedText));
	}
	else
	{
...
sub checkSummarySize
{
	...
	#@words = split(/\s+/, $$text);
	@words = getListOfTokensInText($$text);
	if (   ($lengthLimit == 0 && $byteLimit == 0)
...
sub readText_LCS
{
	...
	if (defined(@{$tokenizedText} > 0))
	{
		for ($t = 0 ; $t < @{$tokenizedText} ; $t++)
		{
			...
			$tokenizedText->[$t] = join(' ', getListOfTokensInText(lc $tokenizedText->[$t]));
...
sub getBEScore
{
	...
	foreach $t (@tokens)
	{
		if ($t ne "_cn_")
		{
			...
			if (exists($peerBEs->{$t}))
			{
				$h = $peerBEs->{$t} <= $modelBEs->{$t} ? $peerBEs->{$t} : $modelBEs->{$t};    # clip
				$$hit += $h;
				if (defined($opt_v))
				{
					print "* Match: $t\n";
...
sub createNGram
{
	...
	@mx_tokens = getListOfTokensInText($text);
	$byteSize  = 0;
...
sub createSkipBigram
{
	...
	for ($i = 0 ; $i < $#m_tokens ; $i++)
	{
		...
		for ($j = $i + 1 ; $j <= $#m_tokens && ($skipDistance < 0 || $j <= $i + $skipDistance + 1) ; $j++)
		{
			$gram = $m_tokens[$i];
			$gram .= " $m_tokens[$j]";
			$count++;
...
sub tokenizeText_LCS
{
	...
	for ($t = 0 ; $t < @{$text} && $done == 0 ; $t++)
	{
		...
		@mx_tokens = getListOfTokensInText($text->[$t]);
			...
			|| $node->getNodeType == DOCUMENT_NODE)
	{
		if ($node->getNodeType == ELEMENT_NODE)
		{
			...
			elsif ($nodeName =~ /^INPUT-FORMAT$/oi)
			{
				...
				if ($inputFormat =~ /^(SEE|ISI|SPL|SIMPLE)$/oi)
				{    # SPL: one sentence per line
					if (exists($ROUGEEvals->{$evalID}))
					{
...
sub stem
{
	...
	{
		$stem = $`;
		if ($stem =~ /$mgr1/o or ($stem =~ /$meq1/o and not $stem =~ /^${C}${v}[^aeiouwxy]$/o)) { $w = $stem; }
	}
...
# functions added by Jeff Kubina are below
...
sub getListOfTokensInText
{
	if (defined $opt_N)
	{
		...
		$text =~ s/^ //;
		$text =~ s/ $//;
		my $last = length($text) - $opt_N;
		return ($text) if $last < 1;
		my @words;
		for (my $i = 0 ; $i <= $last ; $i++)
		{
			push @words, substr($text, $i, $opt_N);
		}
		return @words;
	}
	else
...
__DATA__
ROUGE_EVAL_HOME=/g01/conundrum/DUC/DUC05/results/RELEASE-1.5.5/data
export ROUGE_EVAL_HOME
ROUGE-1.5.5.pl  -x -u -d -m -n 2 -2 4 -c 95 -r 1000 -f A  -t 0 -a -l 100 singles_jackknife.in>singles_jackknife.out

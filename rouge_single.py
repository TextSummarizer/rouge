...
def _create_settings_file(systems_dir_path, gold_standard_path, filename):
    ...
    settings_file = open('settings_log_exp_4.xml', 'w')
    settings_file.write('<ROUGE_EVAL version=\"1.5.7\">\n')
...
def _create_settings_single_file(systems_dir_path, gold_standard_path):
    import os
    models_dir_path = gold_standard_path
...
def _run_rouge_script(script_path, data_path, limit):
    ...
    perl_script = subprocess.Popen(
        ["perl", script_path, "-e " + data_path, "-a", "-n 4", "-u", "-m", "-l " + str(limit),
         "settings_log_exp_4.xml"], stdout=subprocess.PIPE)
    return perl_script.communicate()[0]


def _post_processing(message, storage):
    os.remove('settings_log_exp_4.xml')
...
def generate_settings_file(
        ...
        _create_settings_single_file(system_summary_path, gold_standard_path)
